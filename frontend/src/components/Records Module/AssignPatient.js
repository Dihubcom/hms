...
handleSubmit(e) {
  ...
  render() {
    ...
    return (
      <div>
        <Button onClick={this.toggle}>Asign Doctors to Patients{this.props.buttonLabel}</Button>
        <Modal size="lg" isOpen={this.state.modal} toggle={this.toggle} className={this.props.className}>
          <ModalHeader toggle={this.toggle}>Assign Patient to Doctor</ModalHeader>
          <ModalBody>
            <div className="row">
            <div  className="col-md-2">
                </div>
            <form onSubmit={this.handleSubmit.bind(this)} method="POST" className="col-md-8">
                <div className="form-group">
                    <label>Date</label>
                    <input type='date' ref="date_assigned" onChange={this.logChange} className="form-control" />
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                    ...
                    <input type='text' ref="id" onChange={this.logChange} className="form-control" />
                </div>
            ...
            <table className="table table-hover col-md-push-2 assignPatTable col-md-10">
                ...
                {this.state.patientrecords.map(patientrecords =>
                            ...
                            <td>{patientrecords.firstname} {patientrecords.surname}</td>
                            <td>{patientrecords.age}</td>
                            <td>{patientrecords.assigned_to}</td>
                            <td><button className="btn btn-primary" onClick={this.toggle} style={{ marginBottom: '1rem' }}>Edit </button >
